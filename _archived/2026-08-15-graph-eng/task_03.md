...
- [x] 3.1 DSL `ask` grammar + lint (expect required, responders enum, expiry reachability/durations) + produces derivation
- [x] 3.2 P2 migration (`loop_requests` + wait-kind + 4 event kinds) + canonical suite extensions + sweep-root extension for request refs
- [x] 3.3 Coordinator: ask open (render+freeze prompt/context previews + refs), park as `request` wait, escalation/expiry ladder, atomic cancel coupling
- [x] 3.4 Service: `ListRequests` (ordering/cursor/aggregates), `GetRequest`, `Respond` (per-decision schema dispatch, single-winner admission, provenance with `answered_at`)
- [x] 3.5 `ResponderPolicy` (daemon adapter + injection) unified across approve + respond; self-denial over the spawn chain
- [x] 3.6 Surfaces ×3 + capability `loops.respond` + codegen + `loops.md` tool-table rows + site docs (`running.mdx` triple-table rows, new authoring section)
- [x] 3.7 Config lifecycle for `requests.expire_after`
- [x] 3.8 Implement all assigned tests; run focused task verification (the batch gate is deferred by the execution directive)
...
- [x] UT-010, UT-014 — ask/expiry lint
- [x] UT-055, UT-056, UT-062, UT-063, UT-064, UT-065, UT-066, UT-067, UT-068, UT-069 — open/freeze, per-decision validation, allowlist, tiebreaks, idempotent echo, lanes, responder policy rules
- [x] UT-104, UT-105 — `expire_after` config lifecycle
- [x] UT-107, UT-108 — payload shapes (`answered_at`, no raw refs), redaction/truncation
- [x] UT-111 — atomic cancel transitions + `request_canceled`
- [x] UT-114 — `ResponderPolicy` chain evaluation (starter/child/unrelated/human/cross-workspace/stale)
- [x] IT-002 — admission races (respond×respond, respond×expiry, respond×cancel legs; amend legs land in task_04)
- [x] IT-003 — expiry exactly-once across timer/scan/restart
- [x] IT-005 — respond → namespace exposes the answer downstream
- [x] IT-014 — list filtering/ordering/cursors/aggregates under concurrency
- [x] IT-015 — run cancel → atomic request cancel → bell count drop → respond returns `request_canceled`
- [x] E2E-001 — golden-path ask flow (publish → park → requests → respond → resume → done)
- [x] E2E-012 — expiry across daemon restart fires escalation + route exactly once
