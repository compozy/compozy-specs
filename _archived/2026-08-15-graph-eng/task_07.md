...
- [x] 7.1 P7 migration + `loop_timetravel_ops` ledger with the intent-keyed idempotency rules
- [x] 7.2 Diff service (`timetravel_diff.go`) + payloads + CLI/HTTP/UDS/tool surfaces
- [x] 7.3 Rerun planner (`coordinator_rerun.go`) generalizing requeue + guards + provenance + surfaces
- [x] 7.4 P8 migration + fork seed/start transition through the canonical start path + lineage projections
- [x] 7.5 Capability `loops.timetravel` + `ResponderPolicy` executing-only rule for rerun/fork
- [x] 7.6 Docs co-ship (time-travel page, CLI pages, skill rows, triple-table rows)
- [x] 7.7 Cross-cutting suites: IT-001/IT-004/IT-012/IT-013/IT-016 coverage in the canonical backend suites
- [x] 7.8 Implement all remaining assigned tests; run focused task verification (the batch gate is deferred by the execution directive)
...
- [x] UT-070, UT-071, UT-072, UT-073, UT-074, UT-075, UT-076, UT-077 — rerun planner, guards, intent-keyed idempotency
- [x] UT-078, UT-079, UT-080, UT-081, UT-082, UT-082b — fork seed, overrides, lineage, blob abort, canonical start transition
- [x] UT-083, UT-084, UT-085, UT-086, UT-087, UT-088, UT-089 — diff kinds, carried, summaries, run-diff, guards
- [x] UT-109 — origins (`operator_rerun`, `fork_seed`) + eight event kinds enum parity
- [x] IT-001 — full migration set: fresh/reopen/ahead/integrity/equivalence incl. live-run reopen
- [x] IT-004 — seven-operation HTTP=UDS byte parity
- [x] IT-009 — fork snapshot isolation while source live; concurrency policy
- [x] IT-010 — keyed replay vs keyless fresh-operation semantics
- [x] IT-012 — eight event kinds append + SSE replay/resume + contract/listener lockstep
- [x] IT-013 — capability matrix (respond/timetravel grants, diff ungated, self-denials via shared policy)
- [x] IT-016 — cross-workspace negatives (seven ops, lineage, diff, SSE, `aggregates.pending`)
- [x] E2E-010 — rerun on a terminal run: `operator_rerun` generation, carried nodes intact, completes
- [x] E2E-011 — fork with overrides: linked run, two-way lineage, source byte-identical
- [x] E2E-014 — agent native-tools journey: requests→respond granted; self-response and ungated timetravel denied
...
- Diff, rerun, and fork now share workspace-owned durable history while preserving source runs. Rerun records `operator_rerun`; fork writes a settled generation-1 seed, starts execution at generation 2, and exposes two-way lineage.
- Fresh focused verification passed the new race-enabled GlobalDB and Loop invariants, 4,727 tests across nine affected Go packages in the broader race run, 5,412 web/site tests through Turborepo, both frontend typechecks, `make codegen-check`, and `git diff --check`. The broad race command's RTK output filter reached its 10 MiB limit; the affected time-travel suites were rerun directly and passed.
- The controlling task-loop plan defers `make gate`, live E2E walks, full QA, and review until the post-task tail. The three time-travel scenarios are flagged `untested` for that phase.

Compozy Impact Audit:

- Native tools: added `compozy__loop_diff`, `compozy__loop_rerun`, and `compozy__loop_fork`, including descriptors, request schemas, catalog digests, route tests, and the `loops.timetravel` capability on mutating operations; diff remains ungated.
- Extensibility and hooks: no new hook or bridge SDK surface. Existing loop-start dispatch is reused for new forks; capability registry, native-tool registry, CLI/HTTP/UDS surfaces, docs, and the official skill were updated. Config lifecycle has no new key; checked loop settings and docs for stale time-travel configuration.
- Workspace data isolation: time-travel operations and lineage are workspace-scoped. `workspace_id` is enforced through service, CLI/HTTP/UDS, store reads and writes, event projection, diff, and lineage queries; cross-workspace requests fail closed.
- Official Compozy skill: updated `skills/compozy/references/loops.md` with diff, rerun, fork, lineage, capability, and idempotency behavior.
