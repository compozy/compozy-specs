...
Canonical behavior catalog for the Loop graph-completion feature set: human interaction, routing, fan-out completion strategies, and operator time travel. Companion to `_spec.md`; consumed by `_spec.md` Part II (component mapping), `_uiux.md` (surface states; visual contracts in `docs/design/opendesign/graph-eng/`), and `_tests.md` (coverage matrix).
