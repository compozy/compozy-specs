...
- [ ] 5.1 Grammar + codecs (`StrategySpec`, `StrategyThreshold`, `bind_as`/`index_as`) + lint table
- [ ] 5.2 Pure join-settlement function (`control_join.go`) covering the four strategies with the invariants above
- [ ] 5.3 Settlement → plan integration: cancel intents, collect output/status, `late_arrival`, `branch_pruned`
- [ ] 5.4 P4 migration (completion_state, `partial` status, `branch_pruned`) + terminal-commit projection through every read surface
- [ ] 5.5 `progress.*` namespace projection + alias scoping in `dsl/refs` + history builder
- [ ] 5.6 Iteration-name scoping through fan-out materialization and namespace resolution
- [ ] 5.7 Docs co-ship + editor codec/inspector data updates
- [ ] 5.8 Implement all assigned tests; run `make gate`
...
- [ ] UT-008, UT-009 — iteration-name lint (collision, reserved, scope)
- [ ] UT-012, UT-013 — strategy lint (coverage declaration, thresholds)
- [ ] UT-030, UT-031, UT-032, UT-033, UT-034, UT-035, UT-036, UT-037, UT-037b, UT-038, UT-039, UT-040, UT-041, UT-042, UT-043, UT-044 — settlement semantics + completion_state projection
- [ ] UT-050, UT-051, UT-052, UT-053, UT-054 — progress values, scope rule, nested naming
- [ ] UT-113 — `StrategyThreshold` codec round-trip + rejections
- [ ] IT-007 — fail_fast post-commit lane cancellation (acpmock observes; completed lane intact)
- [ ] E2E-005 — fail_fast cancels running siblings, failure path taken
- [ ] E2E-006 — best_effort partial: collect output + downstream branch + `completion_state: partial` at terminal
- [ ] E2E-007 — race winner settles, losers canceled
