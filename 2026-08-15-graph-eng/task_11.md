...
- [ ] 11.1 Bootstrap the isolated lab (`eng-qa-bootstrap`) and record the manifest
- [ ] 11.2 Walk every in-scope scenario to a recorded verdict (CLI/API tier first)
- [ ] 11.3 Browser tier: request forms, diff view, fork dialog, bell journey via `browser-use:browser`
- [ ] 11.4 Cross-surface comparisons + capability matrix spot checks against persisted state
- [ ] 11.5 Bug registry entries + governed fixes (red-before/green-after) or escalations
- [ ] 11.6 Verdicts + dated report + teardown evidence + `make gate-full`
