...
- [ ] 4.1 Per-lane mutation primitive end to end (store identity, service inputs, post-commit delivery, all surfaces + docs)
- [ ] 4.2 `review:` grammar + lint (decisions/schema pairing, `when`, on_reject reachability) on action nodes
- [ ] 4.3 Coordinator review gating: resolved-params snapshot, request open, held enqueue, admitted-params execution, review-clock rule (pause never consumes the action's own clock)
- [ ] 4.4 Respond decision plumbing for review (approve/edit/reject/respond) over task_03's dispatch
- [ ] 4.5 P3 migration (`loop_node_amendments` + `node_amended`) + sweep-root extension + canonical suites
- [ ] 4.6 Amend service (overlay resolution at every effective-output read) + surfaces + `amendments[]` projection
- [ ] 4.7 Docs co-ship (review authoring section, amend operating section, CLI pages, skill rows)
- [ ] 4.8 Implement all assigned tests; run `make gate`
...
- [ ] UT-011 — review `respond` without output shape lint
- [ ] UT-057, UT-058, UT-059, UT-060, UT-061 — review admission (approve/edit/reject/respond), clock rule
- [ ] UT-099, UT-100, UT-101, UT-102, UT-103, UT-112 — amend overlay, guards, seq CAS, append-only provenance
- [ ] IT-006 — review edit admission → enqueue carries the edited snapshot; executed = admitted
- [ ] IT-011 — amend → resume → downstream consumes effective output; `amendments[]` parity across surfaces
- [ ] IT-017 — blob durability roots (request + amendment refs survive the sweep; true orphans reclaimed)
- [ ] E2E-002 — review flow with edit, executes exactly once with edited args
- [ ] E2E-003 — reject routes via `on_reject.route` without executing
- [ ] E2E-004 — respond-as-result substitutes execution
